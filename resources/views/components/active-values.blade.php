...
<ul {{ $attributes->class('meilifacetsActiveValues') }} aria-label="{{ __('Active filters') }}"
    ...
    @foreach ($values as $value)
        <li class="meilifacetsActiveValue"><button type="button" name="{{ $value->parameter }}" value="{{ $value->value }}" aria-label="{{ $value->action }}" {{ $hook('active-value') }}>{{ $value->label }}<span aria-hidden="true">✕</span></button></li>
    @endforeach
