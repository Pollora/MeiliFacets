...
- [x] 4d-1 · zéro résultat : la barre ne disparaît pas entièrement (`R-49`, fermé le 2026-09-25) — déjà vrai par la décision « valeur tenue affichée même à 0 » (`R-137` #4) ; tests ajoutés, aucun code ; reste la plage de prix tenue sous des bornes vides, masquée (`R-127`), question à Louis
- [x] 4d-2 · « Voir plus » dans un panneau (`R-177`, 2026-09-25, non commité) : dépli en place, focus gardé sur le bouton, panneau borné à `--meili-panel-max`, sheet qui suit ; repli rendu à la fermeture du panneau ou de la section
...
- [x] ANIM-1 · prérequis : `[data-meili][hidden]` sans `!important` sur les briques animées, sorties par `transition-behavior: allow-discrete` + `@starting-style` (`R-89`). **Écart** : la règle est inchangée (`meilifacets.css` l. 7–10, `display: none !important`). L'objectif est pourtant atteint là où une sortie existe — panneaux (`[data-meili="panel"][hidden]`, `display … allow-discrete`), liste du tri, poubelle du pied — parce qu'une transition l'emporte sur `!important` dans la cascade (précision consignée dans `decisions.md`, bloc « Tiroir… », *État ouvert/fermé*). Le tiroir passe par `aria-modal` + `visibility` différée, sans `hidden` (`R-173`). Prérequis sans objet à la lettre : **à clore comme caduc, sur accord de Louis** **Clos comme caduc le 2026-09-25 (Louis)** : une transition l'emporte sur `!important`, les sorties animées fonctionnent sans retirer la règle.
- [x] ANIM-2 · tokens sur `[data-listing]`, surchargeables par le thème : `--meili-ease`, `--meili-ease-drawer`, `--meili-ease-resize`, `--meili-ease-content`, `--meili-duration-resize/content/drawer-in/drawer-out/fade/panel-in/panel-out/hover`, et depuis l'audit `R-48` (lot A) `--meili-duration-press` (150), `-press-pill` (120), `-chevron` (160), `-list` (160), `-pop-in` (200), `-pop-out` (150) — plus aucune durée écrite en dur hors de ces déclarations (verrouillé par `stylesheet.test.ts`, « writes every length on a variable of the listing »). **Écart restant** : le jeton s'appelle `--meili-ease`, pas `--meili-ease-out`
- [x] ANIM-3 · panneau desktop (`R-179`, 2026-09-25, `b02e098`) : entrée `opacity 0 → 1` + `translateY(-4px) scale(0.97)` → `none` par transition CSS (`@starting-style` du bloc `48em`, interruptible : un second clic la retourne), 180 ms `--meili-duration-panel-in`, `--meili-ease` ; sortie 120 ms vers le même état `[hidden]` ; `transform-origin` côté pill inchangé ; `PanelMotion::pop()` ne joue plus de WAAPI. Échap, Tab, pill voisine **et** Entrée/Espace sur la pill (`detail === 0`) : instantanés (`data-instant`). Mouvement réduit : fondu seul 180/120 ms. Mesuré 1440 px, deux modes : 2 `CSSTransition` 180 ms à l'ouverture souris, 3 à 120 ms à la fermeture, 0 animation au clavier, à Échap, à Tab, de pill à pill ; matrice à 89 % de la sortie `scale 0,973, ty −3,55 px` (−4 px visés) ; aucune long task, image max 9,3 ms
...
- [x] ANIM-8 · badge `active-count` : `opacity` 0 + `scale(0.9)`, `--meili-duration-fade`, `--meili-ease`, au seul passage 0 → 1, par WAAPI (`listing/count-entry.ts` à l'époque, `shared/entrance.ts` depuis `R-179`, appelé par `active-count-view.ts`) ; fondu seul en mouvement réduit — `R-173`
- [x] ANIM-9 · pastilles actives (`R-179`, 2026-09-25, `d325a58` + refonte non commitée) : seule une pastille **nouvelle** (identité `kind`/`name`/`value`, lue dans le DOM avant le redessin) entre en `opacity` + `scale(0.95)`, 150 ms `--meili-duration-fade`, `--meili-ease`, par `shared/entrance.ts` (brique commune au badge et aux sections) ; rendu serveur, pastilles reconstruites, plage de prix redessinée : immobiles ; retrait sans sortie, focus rendu à la pastille suivante (`FocusLanding` intact). Mouvement réduit : fondu seul. Mesuré 1440 px, deux modes : 1 `Animation` 150 ms `scale(0.95)` sur la seule nouvelle pastille, 0 sur les autres, 0 après retrait
