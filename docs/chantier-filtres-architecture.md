...
- À partir de 48em (panneau flottant) : un seul panneau ouvert à la fois ; Échap, le clic extérieur et la sortie du focus le ferment, et Échap rend le focus au déclencheur. Passer d'une pill à l'autre, ouvrir ou fermer au clavier se fait sans animation ; l'ouverture souris est une transition CSS depuis `@starting-style` (`R-179`) (`data-instant`, `shared/attributes.ts`). Un panneau qui déborderait du viewport s'aligne sur la fin de son déclencheur (`data-align-end`).
...
- `collapsible/` : `disclosure-group.ts` (motif disclosure APG, exclusivité des panneaux flottants ; l'origine clavier/pointeur lui vient de `shared/input-source.ts`), `panel-motion.ts` (entrée WAAPI, sortie CSS), `toggle-badge-view.ts` (badge de chaque déclencheur).
...
- `listing/` : `active-value-list.ts`, `active-values-view.ts`, `browser-history.ts`, `filter-summary-view.ts`, `focus-landing.ts`, `listing-binding.ts`, `listing-query.ts`, `listing-state.ts`, `listing-url.ts`, `listing.ts`, `new-pills.ts` (quelles pastilles un redessin a amenées, reconnues au filtre qu'elles retirent et non à leur libellé — la vue joue `Entrance` sur elles seules), `reset-focus.ts`, `active-count-view.ts`, `summary-binding.ts` (vues de résumé : total, pastilles, résumé des filtres, `active-count`, badges des déclencheurs), `total-view.ts`.
...
- `results/` : `busy-grid.ts` (`aria-busy` pendant les recherches en vol, `R-179` ; `new BusyGrid(contract).watch(listing)`, branché par `ListingBinding`), `card-view.ts`, `results-view.ts`.
- `shared/` : `attributes.ts` (`EXPANDED`, `INSTANT`), `badge.ts`, `contract.ts` (dont `Contract.window`), `count-label.ts`, `css-timing.ts`, `description.ts`, `entrance.ts` (entrée WAAPI commune : badge, pastilles, sections — `R-179` ; `EntranceStyle` décrit d'où l'élément arrive et les variables qui le rythment), `input-source.ts` (`InputSource` : `InputSource.isKeyboard(click)` — `detail === 0` — et `isPointerDown()`, un appui en cours jusqu'à son clic ; utilisé par `DisclosureGroup` et `ListingBinding`, qui ne lisent plus `detail` eux-mêmes), `filter-expression.ts`, `filter-query.ts`, `plan.ts`, `range.ts`, `search-client.ts`.
