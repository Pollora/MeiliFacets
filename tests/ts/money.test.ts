...
import { Money } from '../../resources/assets/ts/price/money.ts'
...
describe('a price the client writes', () => {
    ...
    /** `number_format()` rounds a half cent up, where `toFixed()` reads the binary value below it. */
    it('rounds a half cent as the shop does', () => {
        assert.equal(new Money(euro).of(1.005), '1,01 €')
    })
